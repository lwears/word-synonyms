...
const (
	...
	GetSynonyms   = `SELECT Synonyms.word_id, Synonyms.synonym_id, Words.word FROM Synonyms 
					LEFT JOIN Words ON Synonyms.synonym_id = Words.id
					WHERE Synonyms.word_id=?;`
	GetWordsForSynonym = `	
					SELECT Synonyms.word_id, Synonyms.synonym_id, Words.word FROM Synonyms 
					LEFT JOIN Words ON Synonyms.word_id = Words.id
